...
function AlertDialogOverlay({ className, ...props }: React.ComponentProps<typeof AlertDialogPrimitive.Overlay>) {
  return (
    <AlertDialogPrimitive.Overlay
      ...
      className={cn('fixed inset-0 z-50 bg-background/80 backdrop-blur-sm animate-in fade-in', className)}
      {...props}
    />
  );
}
  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        ...
        className={cn(
          'fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 rounded-lg border bg-background p-6 shadow-xl duration-200 animate-in fade-in-50 zoom-in-50',
          className,
        )}
        {...props}
      />
